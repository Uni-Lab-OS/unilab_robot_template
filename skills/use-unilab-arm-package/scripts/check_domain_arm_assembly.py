...
def check_domain(domain: Path) -> Report:
    ...
    _check_moveit_attach_contract(domain, arms, report)
    return report
...
def _check_moveit_attach_contract(
    domain: Path,
    arms: list[DeviceModel],
    report: Report,
) -> None:
    """工具附着必须按当前 MoveIt 末端符号解析，不得冻结型号 link。"""

    link_literal = re.compile(
        r"['\"][A-Za-z0-9_]+_(?:cr5|cr7|cr30h)_tool0['\"]"
    )
    asset_roots = [
        domain / "deployment" / "robot_cell" / "assets",
        domain / "szlab_poly_studio" / "deployment" / "robot_cell" / "assets",
    ]
    asset_root = next((root for root in asset_roots if root.is_dir()), asset_roots[0])
    contexts = sorted((asset_root / "tool_contexts").glob("*.y*ml"))
    for path in contexts:
        try:
            document = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        except (OSError, yaml.YAMLError) as error:
            report.errors.append(f"{path.name}: ToolContext 无法读取 ({error})")
            continue
        if not isinstance(document, dict):
            report.errors.append(f"{path.name}: ToolContext 必须是对象")
            continue
        if "mount_link" in document or document.get("mount_link_ref") != "moveit.end_effector":
            report.errors.append(f"{path.name}: 必须使用 mount_link_ref=moveit.end_effector")
        scene = document.get("planning_scene") or {}
        if not isinstance(scene, dict):
            report.errors.append(f"{path.name}: planning_scene 必须是对象")
            continue
        if "parent_link" in scene or scene.get("parent_link_ref") != "moveit.end_effector":
            report.errors.append(f"{path.name}: 必须使用 parent_link_ref=moveit.end_effector")
        refs = scene.get("allowed_touch_link_refs")
        if not isinstance(refs, list) or "moveit.end_effector" not in refs:
            report.errors.append(f"{path.name}: allowed_touch_link_refs 缺少 moveit.end_effector")

    if not contexts:
        report.warnings.append("未找到 ToolContext；无法验证 MoveIt attach 末端引用")
    scan_roots = [
        domain / "deployment" / "robot_cell",
    ...
    for arm in arms:
        for scan_root in scan_roots:
            ...
            for path in scan_root.rglob("*"):
                ...
                for match in link_literal.finditer(text):
                    if path.name in {"moveit_model.py", "model.yaml"}:
                        continue
                    report.errors.append(f"{path.relative_to(domain)} 含型号限定 attach link: {match.group(0)}")
