...
- **领域自有型号**：`provider` 可以是 `<domain_python_package>:build_moveit_model`，但必须：
  - 导出 Robot Module API v1（见 `domain-owned-arm-rail` 阶段 2.2）
  - 用 `unilab-robot-model-kit` 组装 URDF/SRDF/mock control
  - `source_digest` 锁定领域包 `models/model.yaml` 的 `source.sha256`
  - 走 `robot_template` 时：`moveit_runtime_assembly` 的 `arm=_ModuleRef(...).python_package` 与 `robot_module` 同 PR 对齐（见 `domain-owned-arm-rail` 阶段 3.2）
