"""从 distribution 自带 CR5 URDF 提供确定的前向运动学。"""
...
def load_joint_specifications() -> tuple[JointSpecification, ...]:
    """从精确 URDF 返回 CR5 六轴的类型、顺序和 SI 限位。"""
...
def forward_kinematics(
    ...
    for position, joint in zip(positions, chain, strict=True):
        if joint.specification.joint_type not in {
            ...
            raise ValueError("CR5 型号运动链只允许旋转关节")
...
def _load_chain() -> tuple[_KinematicJoint, ...]:
    """解析并缓存于模块生命周期内使用的 CR5 六轴 URDF 运动链。"""
    ...
    for index, canonical_name in enumerate(_CANONICAL_JOINT_NAMES, start=1):
        ...
        if element is None:
            raise ValueError(f"CR5 URDF 缺少 joint{index}")
        joint_type = JointType(str(element.attrib.get("type", "")))
        limit = element.find("limit")
        if limit is None:
            raise ValueError(f"CR5 URDF joint{index} 缺少限位")
        specification = JointSpecification(
            canonical_name,
            joint_type,
...
def _vector(
    ...
    if len(values) != 3:
        raise ValueError(f"CR5 URDF {attribute} 必须包含三个数值")
    return values
